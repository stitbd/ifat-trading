...
class ProductTypeController extends Controller implements HasMiddleware
{
    public static function middleware(): array
    {
        return [
            'auth',
            new Middleware('permission:product_type.view', only: ['index', 'getdata']),
            new Middleware('permission:product_type.create', only: ['store']),
            new Middleware('permission:product_type.edit', only: ['update', 'approve']),
            new Middleware('permission:product_type.delete', only: ['distroy']),
        ];
    }
    ...
    public function index()
    {
        ...
        return view('backend.product_type.index', compact('categories'));
    }
    public function getdata(Request $request)
    {
        if ($request->ajax()) {
            $data = ProductType::with('category')->orderBy('created_at', 'desc')->get();
    ...
    //                     ' . $csrfToken . '
    //                     ' . $method . '
    //                     <button type="submit" class="delete btn btn-danger btn-sm me-2" style="padding:8px;">
    //                     <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 20px; height: 20px;">
    ...
    //                 // status button
    //                 if ($row->status) {
    //                     $statusBtn = '<button data-id="' . $row->id . '" class="status-toggle btn btn-sm btn-success me-2 rounded" style="padding:8px;" title="Active - click to deactivate">
    //                         <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 20px; height: 20px;">
    //                             <path d="M12 19V5M12 5L5 12M12 5L19 12" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
    //                         </svg>
    //                     </button>';
    //                 } else {
    //                     $statusBtn = '<button data-id="' . $row->id . '" class="status-toggle btn btn-sm btn-danger me-2 rounded" style="padding:8px;" title="Inactive - click to activate">
    //                         <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 20px; height: 20px;">
    ...
    public function edit($id)
    {
        ...
        $categories = Category::where('status', 1)->orderBy('name')->get();
        return view('backend.product_type.edit', compact('data', 'categories'));
    }
    ...
    public function distroy($id)
    {
        ...
        if (!$find) {
            Alert::error('Error', 'Product Type not found!');
        ...
        Alert::success('Success', 'Product Type deleted Successful!');
        return redirect()->route('product-type.index');
    }
}
