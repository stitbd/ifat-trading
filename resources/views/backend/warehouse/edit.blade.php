...
<div class="modal-content" style="border-radius: 12px; border: none; overflow: hidden;">
        ...
        <form id="warehouseEditForm" method="POST">
            ...
            <div class="p-4 mb-3" style="background:#fff; border:1px solid #eef0f2; border-radius:10px;">
                ...
                <div class="row">
                    ...
                    <!-- Phone -->
                    <div class="col-md-12 mb-3">
                        <label for="phone" class="form-label fw-bold" style="color:#1e1e2d; font-size:13px;">
                            Phone
                        </label>
                    ...
                    <!-- Address -->
                    <div class="col-md-12 mb-3">
                        <label for="address" class="form-label fw-bold" style="color:#1e1e2d; font-size:13px;">
                            Address
                        </label>
                        <textarea class="form-control" id="address" name="address" rows="3" placeholder="Enter warehouse address"
                            style="border:1px solid #dfe2e8; border-radius:8px; padding:11px 14px; font-size:14px;">{{ $data->address }}</textarea>
            ...
            <input type="hidden" value="{{ $data->id }}" id="warehouse_id">
        </form>
    </div>
    ...
    <div class="modal-footer d-flex justify-content-between align-items-center"
        ...
        <div>
            ...
            <button type="submit" form="warehouseEditForm" class="btn submit-btn"
...
<script>
    $(document).ready(function() {
        $("#warehouseEditForm").on("submit", function(e) {
            ...
            $.ajax({
                url: "{{ route('warehouse.update', ':id') }}".replace(':id', warehouseId),
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href =
                            "{{ route('warehouse.index') }}?added-successfully=" +
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        if (errors.phone) $(".phone-error").text(errors.phone[0]).show();
                        if (errors.address) $(".address-error").text(errors.address[0])
                            .show();
