...
<div class="modal-content" style="border-radius: 12px; border: none; overflow: hidden;">
    ...
    <div class="modal-body" style="padding: 24px; background-color: #fbfbfd;">
        <form id="supplierEditForm" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
                ...
                <div class="row">
                        ...
                        @if ($data->image)
                            <div class="col-md-12 mb-3 text-center">
                                <img src="{{ asset('image/supplier/' . $data->image) }}" alt="{{ $data->name }}"
                                    style="height:80px; width:80px; object-fit:cover; border-radius:8px; border:1px solid #ddd;">
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control" name="company_address" rows="2"
                            style="border:1px solid #dfe2e8; border-radius:8px; padding:11px 14px; font-size:14px;">{{ $data->company_address }}</textarea>
            ...
            <input type="hidden" value="{{ $data->id }}" id="supplier_id">
        </form>
    </div>
    ...
    <div class="modal-footer d-flex justify-content-between align-items-center"
        ...
        <div>
            ...
            <button type="submit" form="supplierEditForm" class="btn submit-btn"
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Not Found',
                            text: xhr.responseJSON?.message || 'Supplier not found!'
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
