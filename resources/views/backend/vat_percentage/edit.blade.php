...
<div class="modal-header" style="background-color: #333333; border-bottom: 1px solid #ccc;">
    <h5 class="modal-title" id="vatPercentageEditModalLabel" style="color: #ffffff;">Edit VAT Percentage</h5>
    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
</div>

<!-- Modal Body -->
<div class="modal-body">
    <form id="vatPercentageEditForm" method="POST">
        @csrf
        @method('put')
        <div class="row">
            <!-- Title -->
            ...
            <div class="col-md-12 mb-3">
                <label for="value" class="form-label fw-bold text-dark">Value (%): <span
                        class="text-danger ml-1">*</span></label>
                <input type="number" step="0.01" min="0" max="100" class="form-control border-dark"
                    id="value" name="value" placeholder="Enter VAT value" value="{{ $data->value }}">
        ...
        <input type="hidden" value="{{ $data->id }}" id="vat_percentage_id">
...
<script>
    $(document).ready(function() {
        $("#vatPercentageEditForm").on("submit", function(e) {

            showLoading();
            e.preventDefault();

            let formData = new FormData(this);
            let vatPercentageId = $("#vat_percentage_id").val();

            $.ajax({
                url: "{{ route('vat-percentage.update', ':id') }}".replace(':id',
                    vatPercentageId),
                type: "POST",
                data: formData,
                processData: false,
                ...
                success: function(response) {
                    if (response.success) {
                        $("#vatPercentageEditForm")[0].reset();
                        $("#vatPercentageEditModal").modal("hide");
