...
@section('title')
    Edit CS - {{ $comparativeStatement->cs_no }}
@endsection
...
@section('content')
        ...
        <div class="app-container container-fluid">
            <div class="admin-page-header">
                <div class="admin-page-header-title">
                    ...
                    <h1>Edit Comparative Statement</h1>
                </div>
            ...
            <form id="csForm">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" class="form-control"
                                    value="{{ $comparativeStatement->createdBy->name ?? '-' }}" readonly>
                            </div>
                            <div class="col-md-4 mb-3">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="date" name="cs_date" id="csDate" class="form-control"
                                    value="{{ \Carbon\Carbon::parse($comparativeStatement->cs_date)->format('Y-m-d') }}"
                                    required>
                ...
                <div class="admin-card mb-4">
                    ...
                    <div style="padding:24px;">
                        ...
                        <div class="mb-3 mt-4">
                            ...
                            <textarea name="remarks" class="form-control" rows="3" placeholder="Input your remarks">{{ $comparativeStatement->remarks }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2 mb-5">
                    ...
                    <button type="submit" class="btn submit-btn"
                        ...
                        <i class="bi bi-check-lg me-1"></i> Update CS
                    </button>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Requisition line items: required_qty, already_covered_qty (অন্য CS থেকে), remaining_qty, current_cs_qty (এই CS তে আগে যা ছিল)
            const requisitionItems = @json($items);

            // Existing groups (এই CS-এ আগে থেকে সেভ করা suppliers) — pre-load এর জন্য
            const existingGroups = @json($existingGroups);

            // Fixed CS Qty per item, keyed by requisition_detail_id.
            let csQtyMap = {};
            requisitionItems.forEach(i => csQtyMap[i.requisition_detail_id] =
                (i.current_cs_qty > 0 ? i.current_cs_qty : null));

            // যদি আগে থেকে কোনো supplier/group থাকে, তাহলে শুরু থেকেই qty locked থাকবে
            let itemsLocked = existingGroups.length > 0;

            // Suppliers, keyed by supplier_id — existingGroups দিয়ে pre-populate করা হচ্ছে
            let csGroups = {};
            existingGroups.forEach(g => {
                csGroups[g.supplier_id] = {
            ...
            */
            function renderIndentDetailsTable() {
                ...
                requisitionItems.forEach(item => {
                    ...
                    let fullyCovered = item.remaining_qty <= 0 && !(qty >
                    0); // remaining_qty already excludes this CS
                    let qtyDisabled = (itemsLocked || fullyCovered) ? 'disabled' : '';
                    let hasQty = qty > 0;
                    let priceDisabled = (hasQty && !fullyCovered) ? '' : 'disabled';

                    // max attribute: remaining_qty (অন্য CS থেকে covered) + এই item এর জন্য এই CS-এ আগের qty
                    let maxQty = item.remaining_qty + (item.current_cs_qty || 0);
                    ...
                    rows += `
                            <tr data-requisition-detail-id="${item.requisition_detail_id}">
                                ...
                                <td>
                                    <input type="number" min="0" max="${maxQty}" step="any"
            ...
            renderComparisonTable(); // existing groups থাকলে শুরুতেই comparison table দেখাবে
            ...
            */
            ...
            function renderComparisonTable() {
                ...
                selected.forEach(item => {
                    ...
                    supplierIds.forEach(sid => {
                        let price = parseFloat(csGroups[sid].prices[item.requisition_detail_id]) ||
                            0;
                        let total = price * qty;
                        bodyRows +=
                ...
                function groupTotal(g) {
                    return selected.reduce((sum, item) => sum + (parseFloat(g.prices[item
            ...
            */
            $('#csForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('comparative-statement.update', $comparativeStatement->id) }}",
                    type: 'POST',
                    data: data,
                    beforeSend: function() {
